...
def generate_fake_points_helper_Algo1(df, threshold_density):
    ...
    x = np.linspace(min(df['X']), max(df['X']), num=250)
    y = np.linspace(min(df['Y']), max(df['Y']), num=250)
    ...
    merged_df = pd.concat([df, filled_df])
    merged_df = merged_df.drop(['x_cell', 'y_cell'], axis=1)
